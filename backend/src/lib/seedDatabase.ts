...
/**
 * Core seed logic – used by both prisma/seed.ts (CLI) and seed.routes.ts (API).
 * Accepts an optional PrismaClient so callers can supply their own instance.
 */
export async function seedDatabase(externalPrisma?: PrismaClient) {
  ...
  try {
    ...
    const wsEngineering = await prisma.workspace.create({ data: { name: 'Engineering', color: '#6366f1', ownerId: sarah.id } });
    const wsStartup = await prisma.workspace.create({ data: { name: 'Startup Ideas', color: '#f59e0b', ownerId: sarah.id } });
    const wsMarketing = await prisma.workspace.create({ data: { name: 'Marketing', color: '#ec4899', ownerId: mike.id } });
    const wsFreelance = await prisma.workspace.create({ data: { name: 'Freelance Projects', color: '#14b8a6', ownerId: mike.id } });
    const wsSideHustle = await prisma.workspace.create({ data: { name: 'Side Hustle', color: '#8b5cf6', ownerId: mike.id } });
    const wsDesign = await prisma.workspace.create({ data: { name: 'Design Studio', color: '#f43f5e', ownerId: emily.id } });
    const wsPersonal = await prisma.workspace.create({ data: { name: 'Personal', color: '#06b6d4', ownerId: emily.id } });
    ...
    const boardProduct = await prisma.board.create({ data: { title: 'Product Roadmap Q1', description: 'Sprint board for Q1 feature releases', color: '#3b82f6', workspaceId: wsEngineering.id, ownerId: sarah.id } });
    const boardBackend = await prisma.board.create({ data: { title: 'Backend API', description: 'REST API endpoints and database work', color: '#10b981', workspaceId: wsEngineering.id, ownerId: sarah.id } });
    const boardMVP = await prisma.board.create({ data: { title: 'MVP Launch', description: 'Minimum viable product planning', color: '#f59e0b', workspaceId: wsStartup.id, ownerId: sarah.id } });
    const boardCampaigns = await prisma.board.create({ data: { title: 'Q1 Campaigns', description: 'Marketing campaigns and outreach', color: '#ec4899', workspaceId: wsMarketing.id, ownerId: mike.id } });
    const boardSocial = await prisma.board.create({ data: { title: 'Social Media Calendar', description: 'Content scheduling for all platforms', color: '#f97316', workspaceId: wsMarketing.id, ownerId: mike.id } });
    const boardClientA = await prisma.board.create({ data: { title: 'Client: Acme Corp', description: 'Website redesign project for Acme', color: '#8b5cf6', workspaceId: wsFreelance.id, ownerId: mike.id } });
    const boardCrypto = await prisma.board.create({ data: { title: 'Crypto Tracker App', description: 'Side project – crypto portfolio app', color: '#eab308', workspaceId: wsSideHustle.id, ownerId: mike.id } });
    const boardUIKit = await prisma.board.create({ data: { title: 'UI Kit v2', description: 'Design system components library', color: '#f43f5e', workspaceId: wsDesign.id, ownerId: emily.id } });
    const boardBranding = await prisma.board.create({ data: { title: 'Rebranding 2026', description: 'Full brand identity refresh', color: '#a855f7', workspaceId: wsDesign.id, ownerId: emily.id } });
    const boardTravel = await prisma.board.create({ data: { title: 'Travel Plans', description: 'Trip planning and bucket list', color: '#06b6d4', workspaceId: wsPersonal.id, ownerId: emily.id } });
