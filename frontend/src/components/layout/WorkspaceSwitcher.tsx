...
import {
  ChevronDown, Plus, Settings, Users, Check, Pencil, Trash2,
} from 'lucide-react';
...
export const WorkspaceSwitcher: React.FC = () => {
  const { workspaces, currentWorkspace, setCurrentWorkspace, createWorkspace, updateWorkspace, deleteWorkspace, fetchWorkspaces } =
    useWorkspaceStore();
  ...
  const handleDeleteWs = async (wsId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (deletingWsId === wsId) {
      try {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="flex items-center gap-2.5 w-full px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors group"
      >
        <div
          ...
          <div className="max-h-48 overflow-y-auto">
            {workspaces.map((ws) => (
              editingWsId === ws.id ? (
                ...
                <div key={ws.id} className="flex items-center gap-2 px-3 py-2 bg-red-50 dark:bg-red-900/20 border-y border-red-200 dark:border-red-800">
                  <span className="text-xs text-red-600 dark:text-red-400 flex-1 truncate">Delete "{ws.name}"?</span>
                  <button
                    onClick={(e) => handleDeleteWs(ws.id, e)}
                    className="text-xs px-2.5 py-1 rounded bg-red-500 text-white hover:bg-red-600 transition-colors font-medium"
                  >
                    Confirm
                  </button>
                  <button
                    onClick={(e) => { e.stopPropagation(); setDeletingWsId(null); }}
              ...
              <div
                ...
                className="flex items-center gap-2.5 px-3 py-2 w-full hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer group/ws"
              >
                <div
                  className="w-7 h-7 rounded-lg flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
                  style={{ backgroundColor: ws.color }}
